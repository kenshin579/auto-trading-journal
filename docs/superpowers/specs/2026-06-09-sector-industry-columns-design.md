...
- **출처**: KIS `client.Domestic.SearchStockInfo(ctx, 종목코드, "300")`
  - 섹터 = `IdxBztpMclsCdName` (지수업종 중분류명, 예 "전기,전자")
  - 산업 = `IdxBztpSclsCdName` (지수업종 소분류명, 더 세부)
  - 한글명 직접 제공 → 매핑 테이블/LLM 불필요
  - ⚠️ **검증 중 교정(2026-06-09)**: `IdxBztpLclsCdName`(대분류)는 업종이 아니라 "시가총액규모"
    (예 "시가총액규모대")를 반환함 → 대분류 미사용. 실제 업종은 **중분류**, 더 세부는 **소분류**.
    (소분류 실제 값은 KIS 토큰 쿨다운 후 E2E 에서 최종 확인 예정.)
- **KIS 토큰 저장소**: atj 는 파일 토큰 캐시 강제(`ensureKISFileToken`) — 사용자 env 의 redis
  설정과 무관(로컬 Redis 미동작 시 토큰 재발급 → KIS 발급제한 회피).
