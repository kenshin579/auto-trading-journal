...
- [x] 실행하여 기존 수동 포맷이 제거되는지 확인 (`--dry-run` 불가, 실제 시트 확인)
...
- [x] `python main.py` 실행
- [x] 대시보드 > 월별 성과: 수익률(%) 컬럼이 `0.00%` 포맷인지 확인
- [x] 대시보드 > 월별 성과: 매수금액/매도금액/실현손익이 `₩#,##0` 포맷인지 확인
- [x] 대시보드 > 투자 지표 > 섹터별 투자비중: `2.26%` 형태로 표시되는지 확인
- [x] 대시보드 > 투자 지표 > 상위 5종목 집중도 / 평균 수익률 / 평균 손실률: `%` 포맷 확인
- [x] 대시보드 > 투자 지표 > 수익 Top 10: `₩#,##0` 포맷 확인
- [x] 대시보드 > 투자 지표 > 손실 Top 10: `₩#,##0` 포맷 확인 (음수도 정상 표시)
